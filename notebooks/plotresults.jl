...
# ╔═╡ cf0b909a-6d1e-4664-9624-270074cd2a62
using UnPack

# ╔═╡ 6c435a2c-2b9a-11ee-3911-a5eefd3d0723
using JLD2, DotEnv

# ╔═╡ 1602f3fb-17a4-48e7-a2a1-ace84662d712
using Plots

# ╔═╡ 5955bff6-34ef-44b8-b2b7-ec632f72c771
using Lux, NeuralPDE

# ╔═╡ 3d2ca960-c2b9-4740-8314-b237b39ca94b
using ModelingToolkit, IntervalSets

# ╔═╡ 30940d7e-b1d1-4414-852f-96251830f77c
using DifferentialEquations

# ╔═╡ d27583eb-ff7c-40e0-ab9a-b1ec48d27c07
...
# ╔═╡ 668468b1-effd-4a91-bf51-e8ab67815a25
default(size = 600 .* (√2, 1), dpi = 180, linewidth = 1.8)

# ╔═╡ 43cf217f-6d55-484a-83d0-4e13e4136dc6
begin
	env = DotEnv.config("../.env")
	DATAPATH = get(env, "DATAPATH", "data")
end;

# ╔═╡ edb39741-7a74-4fca-a4a3-48471eff6d74
span(interval) = supremum(interval) - infimum(interval);

# ╔═╡ de3f87a6-ab0a-49e0-8b3a-78904a9b0e6c
md"## Imports"

# ╔═╡ fda5055e-cb7b-4035-9300-4d34317bb25a
c = ingredients("../src/model/climate.jl");

# ╔═╡ 9e00be90-15e9-48b7-9f3a-001b4cde0304
e = ingredients("../src/model/economy.jl");

# ╔═╡ 18afcf44-8477-4111-b966-8f017030c997
begin
	economy = e.Economy()
	albedo = c.Albedo()
	hogg = c.Hogg()
...
# ╔═╡ 92b8e1e6-015f-421f-8715-dd74a48823ac
md"## Economic model without climate externalities"

# ╔═╡ be02c071-7123-4b6c-b057-cabbf699c9f9
results = joinpath("..", DATAPATH, "nn", "economynn.jld2") |> load;

# ╔═╡ de95e728-98fe-4703-b7c7-72b12b6389d3
@unpack phi, depvar, domains = results;

# ╔═╡ 3ccf5566-bee9-4239-8ca5-bac66c1b7bc4
_, outputdomain = [d.domain for d in domains];

# ╔═╡ 9b63ac33-2567-4374-824a-56cd9e4bc595
timedomain = 0..80;

# ╔═╡ a539041f-f534-4359-9fba-e1b3d8b8ab52
# ╠═╡ show_logs = false
begin
	vₑ(t, y) = first(phi)([t, y], depvar[:v]) |> first
	χₑ(t, y) = last(phi)([t, y], depvar[:χ]) |> first
end;

# ╔═╡ 64def63a-59ae-4fa2-9a95-5455c261b7a4
let
	ar = span(timedomain) / span(outputdomain)
	ts = range(timedomain; step = 1)
	ys = range(outputdomain; length = length(ts))
	
	vfig = contourf(
		ts, ys, vₑ; 
		xlabel = "\$t\$", ylabel = "\$y\$", title = "\$v(t, y)\$",
		xlims = extrema(timedomain), ylims = extrema(outputdomain),
		c = :viridis, aspect_ratio = ar
	)

	χfig = contourf(
		ts, ys, χₑ; 
		xlabel = "\$t\$", ylabel = "\$y\$", title = "\$\\chi(t, y)\$",
		xlims = extrema(timedomain), ylims = extrema(outputdomain),
		c = :viridis, aspect_ratio = ar
	)

	plot(vfig, χfig; size = 450 .* (2√2, 1), margins = 7Plots.mm)
end

# ╔═╡ 6252d0cb-21e3-462e-a6b7-216079449e88
begin
	A(t) = e.A(t, economy)
	ϕ(t, y) = e.ϕ(χₑ(t, y), A(t), economy)
end

# ╔═╡ 7713635e-f7d0-47c2-8588-ee14af4130f5
f(y, p, t) = economy.ϱ + ϕ(t, y) - economy.δₖᵖ ;

# ╔═╡ 4f3f4c41-6493-4547-b736-5147cdc6b8e7
prob = ODEProblem(f, log(economy.Y₀), extrema(timedomain))

# ╔═╡ 00f17a68-b1d4-48bd-bf5c-9224adca8a55
sol = solve(prob);

# ╔═╡ 8419851f-b0da-4b1e-a6f2-c2a3283c4e90
let
	ts = range(timedomain, step = 1 / 12)
	yfig = plot(ts, t -> sol(t) |> exp)
...
julia_version = "1.9.0"
manifest_format = "2.0"
project_hash = "c646374982dda221fc25e6bbfe68d59e198cad1f"
...
[[deps.HTTP]]
deps = ["Base64", "CodecZlib", "ConcurrentUtilities", "Dates", "ExceptionUnwrapping", "Logging", "LoggingExtras", "MbedTLS", "NetworkOptions", "OpenSSL", "Random", "SimplesimilarStream", "Sockets", "URIs", "UUIDs"]
...
git-tree-sha1 = "c5faf13b838ca8935cce0286e3dc6c8a705818fb"
...
[[deps.Lux]]
deps = ["Adapt", "ChainRulesCore", "Functors", "LinearAlgebra", "LuxCUDA", "LuxCore", "LuxLib", "Markdown", "NNlib", "Optimisers", "PackageExtensionCompat", "Random", "Setfield", "SparseArrays", "Statistics", "TruncatedStacktraces", "WeightInitializers"]
git-tree-sha1 = "e7ea74e43e5eec0bf24f7f3df8d2b8529486b885"
...
[[deps.LuxLib]]
deps = ["ChainRulesCore", "KernelAbstractions", "Markdown", "NNlib", "PackageExtensionCompat", "Random", "Reexport", "Statistics"]
git-tree-sha1 = "e9499960327047f846427741cda1ca492b1bcfbe"
...
[[deps.NeuralPDE]]
deps = ["Adapt", "ArrayInterface", "ChainRulesCore", "ComponentArrays", "DiffEqBase", "DiffEqNoiseProcess", "Distributions", "DocStringExtensions", "DomainSets", "Flux", "ForwardDiff", "Integrals", "IntegralsCubature", "LinearAlgebra", "Lux", "ModelingToolkit", "Optim", "Optimisers", "Optimization", "QuasiMonteCarlo", "Random", "RecursiveArrayTools", "Reexport", "RuntimeGeneratedFunctions", "SciMLBase", "Statistics", "StochasticDiffEq", "SymbolicUtils", "Symbolics", "UnPack", "Zygote"]
git-tree-sha1 = "58aca1bce4b71d612ce7ad7f43624e0f1d68c4a2"
...
git-tree-sha1 = "cae442a51e698f985e70e61d5e6c3e1216d754c6"
