...
# ╔═╡ 853651d4-7ce3-4eb6-bc88-d71305e66368
md"
Damages $\gamma$: $(@bind γ Slider(0.01:0.01:1., show_value = true, default = 0.5))
"
...
# ╔═╡ 3aa811c8-402b-47e5-8c03-f93a2b8aeec0
begin
	F(x, p, t) = F!(zeros(2), x, p, t)
	...
	@inbounds function Finv!(dx, x, p, t)
		dx .= -F(x, p, t)
		return dx
	end
end;

# ╔═╡ 3645e043-e037-441c-9a13-f30f5bb4e274
function g(T, E)
	dx = zeros(2); F!(dx, [T, E], p, 0)
	return dx
end;

# ╔═╡ 41e734c1-8201-41dd-8712-c83146c48719
function Nₑ(T)
	roots = find_zeros(E -> g(T, E) |> first, -2, 2)
...
# ╔═╡ c169d35c-d86b-42ce-bcbf-d341c5324e64
begin
	ds = CoupledODEs(F!, zeros(2), p)
	dsinv = CoupledODEs(Finv!, zeros(2), p)
end;

# ╔═╡ 449cc8e9-8f99-4b61-8baf-430e11654deb
begin
	DF(x, p, t) = DF!(zeros(2, 2), x, p, t)
...
begin
	...
	plotvectorfield!(vecfig, T, E, g; rescale = 0.12, alpha = 0.4)


	plot!(vecfig, 
		range(extrema(T)...; length = 101), Nₑ; 
		label = "\$N_E\$", c = :black, linestyle = :dash, linewidth = 2
	)

	colors = [:darkgreen, :darkred]

	for (j, T̄) ∈ enumerate([[0., 0.], [2T₀, 0.]])
	
		λ, V = eigen(DF(T̄, p, 0.))
	
		for (i, λᵢ) ∈ enumerate(λ)
			vᵢ = V[:, i]
			...
			scatter!(vecfig, [T̄[1]], [T̄[2]]; c = colors[j], label = false, markersize = 3)
		end
	end
...
md"
- Number of polluters $n$ $(@bind n Slider(1:20, show_value = true, default = 1))
- Tax $\tau$ $(@bind τ Slider(0:0.01:10, show_value = true, default = 0.))
"
...
begin
	...
	dscomp = CoupledODEs(F!, zeros(2), pcomp)
	dscompinv = CoupledODEs(Finv!, zeros(2), pcomp)

	function gc(T, E)
		dx = zeros(2); F!(dx, [T, E], pcomp, 0)
		return dx
	end

	function Ncₑ(T)
		roots = find_zeros(E -> gc(T, E) |> first, -2, 2)
...
let
	...
	plot!(compfig, 
		range(extrema(T)...; length = 101), Ncₑ; 
		label = "\$N_E\$", c = :black, linestyle = :dash, linewidth = 2
	)
...
git-tree-sha1 = "1c410ad53b3d860e2f041c7683a365e30b24b336"
...
git-tree-sha1 = "37cfe757a9375dba19f31ebf2bb82c2550f73bf1"
...
git-tree-sha1 = "c36d3c313af84f0be7261c1e8f7053b5973351bf"
...
git-tree-sha1 = "974ac437739cfb9455854f281ec3efbc34f3371e"
...
deps = ["AbstractFFTs", "DSP", "DelayEmbeddings", "Distances", "Distributions", "Interpolations", "LinearAlgebra", "LombScargle", "Random", "Requires", "StateSpaceSets", "Statistics", "StatsBase", "Wavelets"]
git-tree-sha1 = "b5d26f0aff97747b5a17f1fc46c63b45e1e3f8c6"
...
git-tree-sha1 = "516462456b9e54f2b94351f51186b3b8aedfb7fd"
