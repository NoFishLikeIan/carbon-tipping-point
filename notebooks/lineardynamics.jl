...
# ╔═╡ 23a56e18-dd2f-11ed-3c3a-070c2dc5f2c4
using DifferentialEquations, DynamicalSystems

# ╔═╡ 658e1f81-feb3-40b7-b033-9cf7ed713b46
using Roots

# ╔═╡ 0231138b-f227-4edf-82c8-f1d7832f5cba
using PlutoUI

# ╔═╡ 727f0d39-4547-43d7-9ac1-8821a9881af3
begin
	using Plots
	default(size = 600 .* (√2, 1), dpi = 180, margins = 5Plots.mm, linewidth = 2)
end

# ╔═╡ 56c5ec39-dabc-4b44-859f-da7a66514384
using LinearAlgebra

# ╔═╡ b23ac36e-aea3-45f0-b39b-f6a71ab3eae1
begin
...
# ╔═╡ a5badb94-6fa7-4165-b2ee-ecd05be17976
begin
...
# ╔═╡ 21987adf-ebfa-49e7-95c7-299b240cce7e
begin # Climate parameters from Mendez and Farazmand
	...
	c₀, x₀ = 410, 289.0150783227231
	fromtemptocarbon(c) = x₀ + a * (c - c₀) / c₀ # using ln(c / c₀) ≈ (c - c₀) / c₀
end;

# ╔═╡ 79d6841f-c31b-4e38-bac0-f108457a3b6d
md"
Carbon tax ``\tau``: $(@bind τ Slider(0:0.01:1, default = 0, show_value = true))
"

# ╔═╡ 20b9739e-7fc5-4590-991e-ac6586862b4e
...
# ╔═╡ e8a0cce2-39e1-4138-831f-9190f2c886c7
begin
	A(τ) = [
		-δ 							1;
		(γ / β₁) * a^2 / c₀^2 		ρ + δ ;
	...
	e₀(τ) = (γ / β₁) * (a / c₀) * (x₀ - xₛ - a) - (ρ + δ) * (β₀ - τ) / β₁

	b₀(τ) = [0; e₀(τ)]
end;

# ╔═╡ 17146575-d419-4dea-81c4-86e13b7db0e6
function F!(du, u, p, t)
	τ = p[1]
	du .= A(τ) * u + b₀(τ)
end;

# ╔═╡ 5c33996d-dfd5-4833-a802-1dc54700b513
function g(c, λ, τ)
	du = zeros(2)
	F!(du, [c, λ], [τ], 0.)
...
# ╔═╡ d2291989-ee68-4fbc-988e-571bd7bde20b
function DF!(J, u, p, t)
	τ = p[1]
	J .= A(τ)
end;

# ╔═╡ 114f4be8-b010-44f0-8b15-e5efd5e826a9
ū = A(τ) \ (-b₀(τ))

# ╔═╡ 9adb1461-3b94-4333-b72b-92905fbd63b0
λ, V = eigen(A(τ));

# ╔═╡ de607db7-e740-4f73-92ab-ea67e9016459
φ(c) = ū[2] + (V[2, 1] / V[1, 1]) * (c - ū[1])

# ╔═╡ 11502e88-2ae2-49ee-8c5b-3483f35db39e
let
	narrows = 22
	cₗ, cᵤ = 0, 1.2c₀
	λₗ, λᵤ = ū[2] - 20, ū[2] + 20

	r = (cᵤ - cₗ) / (λᵤ - λₗ)
	
	xs = range(cₗ, cᵤ; length = narrows)
	ys = range(λₗ, λᵤ; length = narrows)
	vecfig = plotvectorfield(xs, ys, (x, y) -> g(x, y, τ), rescale = 1 / 1000, aspect_ratio = r)

	scatter!(vecfig, [ū[1]], [ū[2]]; label = false, c = :black)
	vline!(vecfig, [c₀]; c = :black, linestyle = :dash, label = "\$c_0\$")

	plot!(vecfig, φ; c = :darkred, label = "Optimal emission path")
...
julia_version = "1.8.1"
manifest_format = "2.0"
project_hash = "a1634bb1a42987b446b050236979cebdf175cbda"
...
git-tree-sha1 = "d04921f0c5da0953fa78771cd291464c3a49ff05"
...
git-tree-sha1 = "039fdc5227912d9a98c1351f4ff808164e47219b"
