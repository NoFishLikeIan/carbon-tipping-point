...
# ╔═╡ 6f6ed7f5-54a1-488c-afe6-50527e182f16
using PlutoUI

# ╔═╡ c2245ff8-d9d4-11ed-3c25-c5b33837e020
using Plots

# ╔═╡ 76216588-663d-4c98-8dc1-88020170d4fe
using Roots

# ╔═╡ d9f4d312-b209-4934-a207-6d5ffd1e446a
using NeuralPDE, Lux, Optimization, OptimizationOptimJL

# ╔═╡ 5150544c-8ce5-48c5-a189-f26d1e613fb0
md"### Albedo process"

# ╔═╡ 6b631062-58ae-42d3-b34c-9c8d54ba1e54
N = 151; # State and action space

# ╔═╡ b40f7767-a2cc-4698-b3c2-2a75f0f593b5
temperaturespace = range(285, 300; length = N);

# ╔═╡ 82002daf-fc5e-4276-8f24-9e6f98c20b74
begin
...
# ╔═╡ e1c2899c-dfe5-4f04-a926-ec0e73477331
plot(temperaturespace, α; xlabel = "Temperature (Kelvin)", ylabel = "Albedo", label = nothing, c = :darkblue)

# ╔═╡ 343cc334-b587-4f25-8a82-3ddc02214195
md"## Temperature dynamics"

# ╔═╡ 3db76dfd-b796-474a-932a-88066e727640
begin
	q₀ = 342
	η = 5.67e-8
	A, S = 20.5, 150
	...
	c₀ = 410
	κₓ = 5e8
	δ = 2.37e-10 # Carbon sink

	μ(x, c) = q₀ * (1 - α(x)) + (S + A * log(c / cₚ)) - η * x^4
end;

# ╔═╡ 8fd8d98e-38ed-4ea9-806e-7d25afdf4fe2
begin
	equilibria = []
	tipping = []
	
	for co₂ ∈ range(cₚ, 2cₚ; length = 2N)
		x̄ = find_zeros(x -> μ(x, co₂), extrema(temperaturespace))
		push!(tipping, length(x̄))
		
		for x̄ᵢ ∈ x̄
			push!(equilibria, [co₂, x̄ᵢ])
		end
	end
end;

# ╔═╡ 99248237-ba3e-4004-9eb8-786b02d99c7c
let
	orbit = hcat(equilibria...)'
	scatter(
		orbit[:, 1], orbit[:, 2]; 
		markersize = 1, label = nothing,
		xlabel = "Carbon concentration", ylabel = "Temperature (Kelvin)", margins = 5Plots.mm
	)

	vline!([c₀]; c = :darkred, linestyle = :dash, linewidth = 2, label = "co\$_2(0)\$")
	
end

# ╔═╡ 5afcd87e-11e6-41c2-a8f6-ac084cbe509c
cₜ = range(cₚ, 2cₚ; length = 2N)[findlast(t -> t > 1, tipping)];

# ╔═╡ 536f66a8-2c1a-4413-87f0-2feffaf3a427
md"
## Optimal emissions
"

# ╔═╡ cb4daa6a-bf24-4508-bc75-936c516e1ea9
begin
	x₀ = first(find_zeros(x -> μ(x, c₀), cₚ, 2cₚ))
	...
	e(pc) = clamp(inv(τ - pc), 1e-3, 100)
end;

# ╔═╡ 67d29efe-bd70-45e2-b600-4fd4858a2a3b
l(x, e) = u(e) - τ * e - d(x);

# ╔═╡ 5a5c31f3-b90f-4340-a54b-3b5bcd6894b4
H(x, c, px, pc) = u(e(pc)) - 1 - d(x) + px * μ(x, c) * κₓ - pc * δ * c;

# ╔═╡ 870be94d-8d9c-49a7-90bb-5eb9209d7c14
function hbj(state::NamedTuple, value::NamedTuple)
	(; x, c) = state
	(; v, vx_up, vx_down, vc_up, vc_down) = value
...
# ╔═╡ fdfdc428-8e31-4ccb-b61f-ae5b33f16bee
md"## Single regime"

# ╔═╡ b07f1936-251d-47de-82cf-fda263bb07c6
csingle = (cₜ + 0.01, cₜ + 100); # Single regime region

# ╔═╡ 2aaf394a-22c1-4d10-b37e-551921126c7a
begin
	carbongridhigh = range(csingle...; length = N) |> collect
	temperaturegridhigh = [find_zero(x -> μ(x, c), extrema(temperaturespace)) for c ∈ carbongridhigh]

	vguess = (tup -> l(tup[1], tup[2] * δ)).(Iterators.product(temperaturegridhigh, carbongridhigh))
end;

# ╔═╡ 20ba8f2b-3f93-4548-a040-a42171154f73
pdesolve(hbj, 
	OrderedDict(:x => temperaturegridhigh, :c => carbongridhigh),
	OrderedDict(:v => vguess)
)
...
julia_version = "1.8.1"
manifest_format = "2.0"
project_hash = "72a1e31de515d5b60aaa8d895b6c624546f97e9e"
...
deps = ["ArrayInterface", "ChainRulesCore", "ConstructionBase", "ForwardDiff", "GPUArrays", "LinearAlgebra", "RecursiveArrayTools", "Requires", "ReverseDiff", "SciMLBase", "StaticArrayInterface", "StaticArrays"]
git-tree-sha1 = "bbc7dd1b536d2950c2a91801283c609ef74e72fe"
...
git-tree-sha1 = "d15592ecf6abdc1942752d29fade6e0743b6c2e4"
