...
\subsubsection{Log-separable aggregator}

I further assume that $\psi \to 1$. This implies that the Epstein-Zin aggregator reduces in its log-separable form (\ref{eq:epstein-zin:log-separable}). In turn, letting consumption be $\control{\chi} Y$, the aggregator simplifies to \begin{equation} \label{eq:epstein-zin:log-separable-simplified}
    f(\control{\chi}, y, U) = \rho U \Big( (1 - \theta) \log \control{\chi} + (1 - \theta) y + \log\big((1 - \theta) U\big) \Big).
\end{equation} 

\subsection{Adjustment costs}

Finally, I will assume that capital does not incur in any adjustment cost $\kappa = 0$.
